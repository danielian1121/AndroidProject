...
//        getWindow().setFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN, WindowManager.LayoutParams.FLAG_FULLSCREEN);
        ...
        mSocket.connect();
        String check = bundle.getString("flag");
        if(check.equals("POI")){
            model = getIntent().getParcelableExtra("POI-Content");
            mSocket.emit("share",model.getPOIId());
            mSocket.emit("ios_send_poi",model.getPOIId());
        }else if(check.equals("LOI_to_POI")){
            loiSequence = getIntent().getParcelableExtra("POI-Content");
            mSocket.emit("LoiShare",loiSequence.getPOIId());
            mSocket.emit("ios_send_xoi",loiSequence.getPOIId());
        }
